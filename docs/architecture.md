...
| `users/<u>/metadata.json` | Operator edits on disk (no UI yet) | Per-route overrides keyed by `Region/Trail name`. Optional. |
...
| `site-config.json` | Operator edits via Settings | Site-wide: branding, default map view, category labels, feature flags. |
...
- **Auth.** Cookie-based sessions (`HttpOnly`, `SameSite=Lax`). PBKDF2-SHA256 password hashing. Per-IP rate limit on login. First user to register becomes operator; further registrations require the operator to flip the site-wide registration toggle. See [SECURITY.md](../SECURITY.md) for the full account model.
