...
|---|---|---|
| `users/<u>/places.json` | API writes, frontend reads via `/api/places` | That user's place pins. Category and region are both optional; visited places can carry date_visited / rating. |
| `users/<u>/routes.json` | Generated by `gpx-manifest.sh` (per-user invocation) | Manifest of that user's GPX trails: filename, region, bounding box, computed length and elevation stats, plus any per-trail overrides from `metadata.json` (source URL, date hiked, rating, notes, tags, difficulty, native name). |
| `users/<u>/gpx/<Region>/<Trail>.gpx` | API writes (uploads), manifest reads | Raw GPX trail files. Region directory is optional; root-level files are listed under the "No region" group. |
| `users/<u>/metadata.json` | User edits via the trail Edit modal (or directly on disk) | Per-route overrides keyed by `Region/Trail name` (or just `Trail name` for no-region trails). Optional. |
