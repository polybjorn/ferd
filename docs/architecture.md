...
| `users/<u>/routes.json` | Generated by `gpx-manifest.sh` (per-user invocation) | Manifest of that user's GPX trails: filename, region, computed length and elevation stats. |
