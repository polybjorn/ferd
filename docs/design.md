...
| Auth hardening | Login rate-limiting, optional TOTP 2FA, HIBP k-anonymity password-breach check at register / change-password. | Current threat model is "stranger guesses your password" against a 12-char minimum on a self-hosted single-user app behind Tailscale. HIBP adds a third-party request (currently zero). 2FA needs setup flow + recovery codes + extra schema. Revisit if Atlas grows beyond friends or leaves the Tailscale-protected origin. |
