...
| `GET` | `/metadata` | - | Per-route overrides keyed by `Region/Route` (or just `Route` for no-region). |
| `PUT` | `/metadata` | `{key, metadata}` | Upsert one entry; empty `metadata` deletes the key. See [Common shapes](#common-shapes) for fields. Triggers manifest regen. |
...
| `DELETE` | `/gpx` | `{name, region?}` | Delete a route (and its `.planned.gpx` sibling if present). Prunes empty region dir. Triggers manifest regen. |
| `POST` | `/gpx/move` | `{key, new_region, new_name?}` | Move/rename a route. `new_region: ""` moves to root. Re-keys `metadata.json`. `409` on target conflict. |
...
| `GET` | `/me/export` | - | Returns a zip of the user's folder (`places.json`, `routes.json`, `metadata.json`, `prefs.json`, `category-labels.json`, `gpx/...`). Excludes dotfiles, backup files (`*.bak`, `*~`), and OS metadata (`.DS_Store`, `Thumbs.db`). |
| `POST` | `/me/import?mode=replace\|merge\|prefs` | zip body | `Content-Type: application/zip`. `replace` deletes existing top-level files and the `gpx/` tree before writing; `merge` appends places by unique name and updates metadata/prefs/labels by key; `prefs` only merges `prefs.json` + `category-labels.json` and skips places/routes/metadata/GPX even if they're in the archive. GPX files always have PII stripped on import. Triggers manifest regen. |
...
| Method | Path | Body | Notes |
|---|---|---|---|
| `GET` | `/admin/users` | - | List every user with stats (`places`, `routes`, active `sessions`, `published`, `is_admin`). |
| `GET` | `/admin/stats` | - | Site-wide counters: `users`, `published_users`, `places`, `routes`, `db_bytes`, `data_bytes`. |
...
| `POST` | `/admin/catalog/add` | `{entries: [place, ...]}` | Append places to the local catalog (see [catalog.md](catalog.md)). Dedup by `name`. Returns `{added, skipped, total}`. |
| `POST` | `/admin/catalog/delete` | `{names: ["...", ...]}` | Remove catalog entries by name. Returns `{removed, total}`. |
| `POST` | `/admin/catalog/clear` | - | Empty the local catalog. Returns `{removed, total: 0}`. |
| `POST` | `/admin/catalog/hide` | `{name: str, hidden: bool}` | Hide (or restore) one shipped catalog entry. Hidden entries are filtered from `/api/catalog` for everyone. |
...
| `GET` | `/u/<username>/routes` | Route manifest. |
| `GET` | `/u/<username>/metadata` | Per-route overrides. |
...
End-to-end sequences for manually verifying auth and writes against a local dev server.

Auth (fresh server, register, login):
...
Writes (place CRUD and GPX upload/delete):
