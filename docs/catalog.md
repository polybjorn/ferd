...
- `tags` - optional array of free-form labels (`[a-zA-Z0-9][a-zA-Z0-9-]{0,31}`, max 10, deduped case-insensitively so `UNESCO` keeps its casing), e.g. `["UNESCO"]`. Use for cross-cutting attributes that aren't the single `category` (a place is one category but can carry several tags). Natural-feature type lives here rather than as its own category: `nature` entries carry `volcano`, `waterfall`, or `cave` so the feature stays filterable without a separate marker color. Imported and update-tracked like the other catalog fields.
