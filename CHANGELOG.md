...
### Fixed
- Maps with routes visible are no longer sluggish to pan or pinch-zoom on touch devices: the per-frame route reprojection (needed only on desktop smooth-wheel zoom) is replaced on touch by the native pan/zoom plus a cheap per-frame stroke-width counter-scale, so route lines hold a constant width through a pinch without rebuilding geometry, then reproject crisp when the gesture ends. Desktop also skips reprojection on pure pans.
